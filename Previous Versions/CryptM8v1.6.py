#CryptM8 v1.6 J.Street 19/04/2019 'Jordanstreet@protonmail.com'
...
def encrypt_drive():
    print("WARNING MODULE HEAVILY UNDERTESTED")
...
def decrypt_drive():
    ...
    drives = drives.split('\000')[:-1]
    print("Current mounted drives are: " + str(drives))
    dm_drive = input("Enter the drive you wish to decrypt: ")
    for root, dirs, files in os.walk(dm_drive):
        for name in files:
            no_files = no_files + 1
            print(os.path.join(root, name))
...
def menu():
    ...
    print("-----------------------------------------------------------------------------------------------------------")
    print("")
    print("Welcome to CryptM8 v1.6 19/04/19" + Style.RESET_ALL)
    ...
    print("Current date/time is: " + now.strftime("%H:%M %Y-%m-%d"))
    print(Fore.GREEN)
    print("-----------------------------------------------------------------------------------------------------------")
    print(Style.RESET_ALL)
    print("Browse(B)|Encrypt File(E)|Encrypt Drive(ED)|Encrypt Folder(EF)|Decrypt File(D)|Decrypt Drive (DD)|Decrypt Folder(DF)")
    print("Generate Key(G)|Delete Current K/S (KS)|File Send (FS)|File Receive(FR)|Quit (Q)")
    ...
    elif usrchoice != "B" or "DF" or "E" or "D" or "G" or "EF" or "Q" or "KS" or "DF" or "FS" or "FR" or "u,u,d,d,l,r,l,r,b,a,s":
        print("You entered an invalid choice...")
        menu()

def made_by():
    progress_spin = Spinner()
    import time
    first_line = "CryptM8 v1.6 (19/04/19) developed by Jordan Street"
    ...
    third_line = "P.S This is my first proper Python/programming project, and as such I know there is a huge number of newbie errors \n" \
                 "and program breaking bugs however please be kind. As I will continue working on CryptM8 in my sparetime.\n" \
