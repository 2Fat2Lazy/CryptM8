#CryptM8 v1.7 J.Street 23/05/2019 'Jordanstreet@protonmail.com' (Distributed under GNUAGPLv3 Licence)
...
def user_auth():
    ...
    if user_key_exists == True:
        print("This version of CryptM8 is secured with a user key, please enter it below to access the full encryption suite")
        ...
        if hashed_key != saved_key:
            print("Hash mismatch! This is due to a wrong password! Please try again.")
            user_auth()
        if hashed_key == saved_key:
    ...
    if user_key_exists == False:
        print("CryptM8 password not currently set, this will be required every time you start CryptM8, please enter one"
              " below...")
        user_key = input("Enter a user password: ")
        user_salt = base64.b64encode(CM8_mast_salt)
        kdf = PBKDF2HMAC(
            algorithm=hashes.SHA3_224(),
            length=32,
        ...
        print("Hashing password, please wait...")
        hashed_key = base64.urlsafe_b64encode(kdf.derive(user_key.encode()))
...
def secret_menu():
    ...
    print("Menu|Main Menu (Q)|Credits (C)|Game (G)|")
...
def menu():
    ...
    print("-----------------------------------------------------------------------------------------------------------")
    print("")
    print("Welcome to CryptM8 v1.7 23/05/19" + Style.RESET_ALL)
    ...
    elif usrchoice != "A" or "B" or "DF" or "HD" or "UH" or "E" or "D" or "G" or "EF" or "Q" or "KS" or "DF" or "FS" or "FR" or "uuddlrlrbas":
        print("You entered an invalid choice...")
        input("Press 'Enter' to return to main menu.")
        menu()

def game():
    dec_key = "8vAeeNVmFCqMxn-x1nOwlfnD5NZHyZgthcfi-1tmAso="
    with open("Test.CM8", 'rb') as f:
        data = f.read()
        try:
            fernet = Fernet(dec_key)
            decrypted = fernet.decrypt(data)
        except:
            print("Unknown error ", sys.exc_info()[0], " occurred.")
            print("If you have not downloaded the whole distribution or have files missing this error may occur.")
            input("Press 'Enter' to return to decryption menu.")
            decrypt()
...
def made_by():
    import time
    first_line = "CryptM8 v1.7 (23/05/19) developed by Jordan Street(Distributed under GNU AGPLv3 Licence)"
    second_line = "If you modify/re-use my code please credit/say thanks as it has been a slow process " \
                  "over a couple months or so to get to this stage\n and would mean the world to me. Thanks"
